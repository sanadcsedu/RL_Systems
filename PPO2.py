# import gym
#This one tries to train a single model from all users' interactions
...
class PPO(nn.Module):
    ...
    def make_batch(self):
        ...
        s,a,r,s_prime,done_mask,prob_a = torch.tensor(np.array(s_lst), dtype=torch.float), torch.tensor(np.array(a_lst)), \
...
def main(filename, model):
    ...
    env = environment()
    print("Max Possible Reward: ", env.make(filename))
    score = 0.0
    print_interval = 20
